...
package projetsid.dumpcustomergroup_0_1;
...
/**
 * Job: dumpCustomerGroup Purpose: load le product<br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class dumpCustomerGroup implements TalendJob {
	...
	private final String jobName = "dumpCustomerGroup";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					dumpCustomerGroup.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(dumpCustomerGroup.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:kRSJs21Kg0J4NnhliOmVja5Ry//LkXOZAnl0C/YXe+AjkJn5XagfyYbpO4xs0t+aVVcoemofH8vO0kr/");
	...
	public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
		final static byte[] commonByteArrayLock_PROJETSID_dumpCustomerGroup = new byte[0];
		static byte[] commonByteArray_PROJETSID_dumpCustomerGroup = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PROJETSID_dumpCustomerGroup.length) {
					if (length < 1024 && commonByteArray_PROJETSID_dumpCustomerGroup.length == 0) {
						commonByteArray_PROJETSID_dumpCustomerGroup = new byte[1024];
					} else {
						commonByteArray_PROJETSID_dumpCustomerGroup = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PROJETSID_dumpCustomerGroup, 0, length);
				strReturn = new String(commonByteArray_PROJETSID_dumpCustomerGroup, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PROJETSID_dumpCustomerGroup, 0, length);
				strReturn = new String(commonByteArray_PROJETSID_dumpCustomerGroup, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PROJETSID_dumpCustomerGroup) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = ("Customer_Group");
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "\".\"" + ("Customer_Group");
				}
				...
				try (java.sql.ResultSet rsTable_tDBOutput_1 = dbMetaData_tDBOutput_1.getTables(null, null, null,
					...
					while (rsTable_tDBOutput_1.next()) {
						...
						if (table_tDBOutput_1.equals(("Customer_Group"))
				...
				try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
					stmtCreate_tDBOutput_1.execute("CREATE TABLE \"" + tableName_tDBOutput_1
							+ "\"(\"Customer_Group_ID\" VARCHAR(2)   not null ,\"Customer_Group\" VARCHAR(40)   not null ,primary key(\"Customer_Group_ID\"))");
				}
				java.sql.PreparedStatement pstmt_tDBOutput_1 = conn_tDBOutput_1.prepareStatement(
						"SELECT COUNT(1) FROM \"" + tableName_tDBOutput_1 + "\" WHERE \"Customer_Group_ID\" = ?");
				resourceMap.put("pstmt_tDBOutput_1", pstmt_tDBOutput_1);
				String insert_tDBOutput_1 = "INSERT INTO \"" + tableName_tDBOutput_1
						+ "\" (\"Customer_Group_ID\",\"Customer_Group\") VALUES (?,?)";
				...
				String update_tDBOutput_1 = "UPDATE \"" + tableName_tDBOutput_1
						+ "\" SET \"Customer_Group\" = ? WHERE \"Customer_Group_ID\" = ?";
...
///////////////////////    			
				...
				try {
					...
					Object filename_tFileInputDelimited_1 = context.folderName
							+ "/files/TablesToDump/Customer_Group.csv";
					...
					try {
						fid_tFileInputDelimited_1 = new org.talend.fileprocess.FileInputDelimited(
								context.folderName + "/files/TablesToDump/Customer_Group.csv", "UTF-8", ";", "\n", true,
...
///////////////////////		
							...
							if (row1.Customer_Group != null) { //

								strBuffer_tLogRow_1.append(String.valueOf(row1.Customer_Group));
...
///////////////////////    			
							...
							if (row2.Customer_Group_ID == null) {
								...
								pstmt_tDBOutput_1.setString(1, row2.Customer_Group_ID);
							}
							...
							if (checkCount_tDBOutput_1 > 0) {
								if (row2.Customer_Group == null) {
									...
									pstmtUpdate_tDBOutput_1.setString(1, row2.Customer_Group);
								}

								if (row2.Customer_Group_ID == null) {
									pstmtUpdate_tDBOutput_1.setNull(2 + count_tDBOutput_1, java.sql.Types.VARCHAR);
								} else {
									pstmtUpdate_tDBOutput_1.setString(2 + count_tDBOutput_1, row2.Customer_Group_ID);
								}
								...
								if (row2.Customer_Group_ID == null) {
									...
									pstmtInsert_tDBOutput_1.setString(1, row2.Customer_Group_ID);
								}

								if (row2.Customer_Group == null) {
									pstmtInsert_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
								} else {
									pstmtInsert_tDBOutput_1.setString(2, row2.Customer_Group);
								}
					...
					if (!((Object) (context.folderName
							+ "/files/TablesToDump/Customer_Group.csv") instanceof java.io.InputStream)) {
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final dumpCustomerGroup dumpCustomerGroupClass = new dumpCustomerGroup();

		int exitCode = dumpCustomerGroupClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = dumpCustomerGroup.class.getClassLoader()
					.getResourceAsStream("projetsid/dumpcustomergroup_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = dumpCustomerGroup.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : dumpCustomerGroup");
		}
