...
package projetsid.dumporder_0_2;
...
/**
 * Job: dumpOrder Purpose: load le product<br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class dumpOrder implements TalendJob {
...
public String getFolderName(){
	...
	private final String jobName = "dumpOrder";
		...
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(dumpOrder.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public void tDBConnection_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
	...
	final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:23WGM8FDH/kadTxdQAHIeAm9ilHW+mskr/ajz20YxiFAkhcAUetOLDg5Fsa34L8ecRIKsIQM33SIcSxS");
...
public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
    final static byte[] commonByteArrayLock_PROJETSID_dumpOrder = new byte[0];
    static byte[] commonByteArray_PROJETSID_dumpOrder = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_PROJETSID_dumpOrder.length) {
				if(length < 1024 && commonByteArray_PROJETSID_dumpOrder.length == 0) {
   					commonByteArray_PROJETSID_dumpOrder = new byte[1024];
				} else {
   					commonByteArray_PROJETSID_dumpOrder = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_PROJETSID_dumpOrder, 0, length);
			strReturn = new String(commonByteArray_PROJETSID_dumpOrder, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_PROJETSID_dumpOrder, 0, length);
			strReturn = new String(commonByteArray_PROJETSID_dumpOrder, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_PROJETSID_dumpOrder) {
...
if(dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
	tableName_tDBOutput_1 = ("Order");
} else {
	tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "\".\"" + ("Order");
}
...
int count_tDBOutput_1=0;
                                ...
                                try (java.sql.ResultSet rsTable_tDBOutput_1 = dbMetaData_tDBOutput_1.getTables(null, null, null, new String[]{"TABLE"})) {
                                    ...
                                    while(rsTable_tDBOutput_1.next()) {
                                        ...
                                        if(table_tDBOutput_1.equals(("Order"))
                                ...
                                if(!whetherExist_tDBOutput_1) {
                                    try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
                                        stmtCreate_tDBOutput_1.execute("CREATE TABLE \"" + tableName_tDBOutput_1 + "\"(\"Order_ID\" VARCHAR(20)  ,\"Order_Type\" VARCHAR(20)  ,\"Order_Date\" TIMESTAMP ,\"Delivery_Date\" TIMESTAMP ,\"Country\" VARCHAR(20)  ,\"Total_Retail_Price\" FLOAT4 ,primary key(\"Order_ID\"))");
                                    }
                                }
	    ...
	    String insert_tDBOutput_1 = "INSERT INTO \"" + tableName_tDBOutput_1 + "\" (\"Order_ID\",\"Order_Type\",\"Order_Date\",\"Delivery_Date\",\"Country\",\"Total_Retail_Price\") VALUES (?,?,?,?,?,?)";
	    java.sql.PreparedStatement pstmtInsert_tDBOutput_1 = conn_tDBOutput_1.prepareStatement(insert_tDBOutput_1);
	    resourceMap.put("pstmtInsert_tDBOutput_1", pstmtInsert_tDBOutput_1);
	    String update_tDBOutput_1 = "UPDATE \"" + tableName_tDBOutput_1 + "\" SET \"Order_Type\" = ?,\"Order_Date\" = ?,\"Delivery_Date\" = ?,\"Country\" = ?,\"Total_Retail_Price\" = ? WHERE \"Order_ID\" = ?";
 ...
 */
	...
	final routines.system.RowState rowstate_tFileInputDelimited_1 = new routines.system.RowState();
				...
				try{
						...
						Object filename_tFileInputDelimited_1 = context.folderName+"/files/TablesToDump/Order.csv";
			...
			if(footer_value_tFileInputDelimited_1 >0 || random_value_tFileInputDelimited_1 > 0){
				throw new java.lang.Exception("When the input source is a stream,footer and random shouldn't be bigger than 0.");				
						...
						try {
							fid_tFileInputDelimited_1 = new org.talend.fileprocess.FileInputDelimited(context.folderName+"/files/TablesToDump/Order.csv", "UTF-8",";","\n",true,1,0,
...
globalMap.put("tFileInputDelimited_1_ERROR_MESSAGE",ex_tFileInputDelimited_1.getMessage());
									rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format("Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
										"Order_Date", "row1", temp, ex_tFileInputDelimited_1), ex_tFileInputDelimited_1));
								}
...
globalMap.put("tFileInputDelimited_1_ERROR_MESSAGE",ex_tFileInputDelimited_1.getMessage());
									rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format("Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
										"Delivery_Date", "row1", temp, ex_tFileInputDelimited_1), ex_tFileInputDelimited_1));
								}
...
} else {pstmtUpdate_tDBOutput_1.setString(1, row2.Order_Type);
}

                        if(row2.Order_Date != null) {
pstmtUpdate_tDBOutput_1.setTimestamp(2, new java.sql.Timestamp(row2.Order_Date.getTime()));
} else {
pstmtUpdate_tDBOutput_1.setNull(2, java.sql.Types.TIMESTAMP);
}

                        if(row2.Delivery_Date != null) {
pstmtUpdate_tDBOutput_1.setTimestamp(3, new java.sql.Timestamp(row2.Delivery_Date.getTime()));
} else {
...
} else {pstmtUpdate_tDBOutput_1.setString(4, row2.Country);
}

                        if(row2.Total_Retail_Price == null) {
pstmtUpdate_tDBOutput_1.setNull(5, java.sql.Types.FLOAT);
} else {pstmtUpdate_tDBOutput_1.setFloat(5, row2.Total_Retail_Price);
}

                        if(row2.Order_ID == null) {
pstmtUpdate_tDBOutput_1.setNull(6 + count_tDBOutput_1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(6 + count_tDBOutput_1, row2.Order_ID);
}
...
} else {pstmtInsert_tDBOutput_1.setString(2, row2.Order_Type);
}

                        if(row2.Order_Date != null) {
pstmtInsert_tDBOutput_1.setTimestamp(3, new java.sql.Timestamp(row2.Order_Date.getTime()));
} else {
pstmtInsert_tDBOutput_1.setNull(3, java.sql.Types.TIMESTAMP);
}

                        if(row2.Delivery_Date != null) {
pstmtInsert_tDBOutput_1.setTimestamp(4, new java.sql.Timestamp(row2.Delivery_Date.getTime()));
} else {
...
} else {pstmtInsert_tDBOutput_1.setString(5, row2.Country);
}

                        if(row2.Total_Retail_Price == null) {
pstmtInsert_tDBOutput_1.setNull(6, java.sql.Types.FLOAT);
} else {pstmtInsert_tDBOutput_1.setFloat(6, row2.Total_Retail_Price);
}
 ...
 */
	...
	currentComponent="tFileInputDelimited_1";
                ...
                if(!((Object)(context.folderName+"/files/TablesToDump/Order.csv") instanceof java.io.InputStream)){
 ...
 */
    ...
    public static void main(String[] args){
        final dumpOrder dumpOrderClass = new dumpOrder();

        int exitCode = dumpOrderClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = dumpOrder.class.getClassLoader().getResourceAsStream("projetsid/dumporder_0_2/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = dumpOrder.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : dumpOrder");
        }
...
/************************************************************************************************
 *     84385 characters generated by Talend Open Studio for Data Integration 
