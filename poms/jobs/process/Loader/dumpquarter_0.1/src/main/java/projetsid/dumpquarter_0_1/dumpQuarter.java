...
package projetsid.dumpquarter_0_1;
...
/**
 * Job: dumpQuarter Purpose: load le product<br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class dumpQuarter implements TalendJob {
	...
	private final String jobName = "dumpQuarter";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(dumpQuarter.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:wwluOsPEHgLXN782p8neT6G4XGXWdfTC553QyHHH47var68JVhak6HfXESCH747b3gbR8gQmOdkj38xU");
	...
	public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
		final static byte[] commonByteArrayLock_PROJETSID_dumpQuarter = new byte[0];
		static byte[] commonByteArray_PROJETSID_dumpQuarter = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PROJETSID_dumpQuarter.length) {
					if (length < 1024 && commonByteArray_PROJETSID_dumpQuarter.length == 0) {
						commonByteArray_PROJETSID_dumpQuarter = new byte[1024];
					} else {
						commonByteArray_PROJETSID_dumpQuarter = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PROJETSID_dumpQuarter, 0, length);
				strReturn = new String(commonByteArray_PROJETSID_dumpQuarter, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PROJETSID_dumpQuarter, 0, length);
				strReturn = new String(commonByteArray_PROJETSID_dumpQuarter, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PROJETSID_dumpQuarter) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = ("Quarter");
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "\".\"" + ("Quarter");
				}
				...
				try (java.sql.ResultSet rsTable_tDBOutput_1 = dbMetaData_tDBOutput_1.getTables(null, null, null,
					...
					while (rsTable_tDBOutput_1.next()) {
						...
						if (table_tDBOutput_1.equals(("Quarter")) && (schema_tDBOutput_1.equals(dbschema_tDBOutput_1)
				...
				if (!whetherExist_tDBOutput_1) {
					try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
						stmtCreate_tDBOutput_1.execute("CREATE TABLE \"" + tableName_tDBOutput_1
								+ "\"(\"id\" VARCHAR(7)   not null ,\"quarterName\" VARCHAR(2)   not null ,\"year\" INT4  not null ,primary key(\"id\"))");
					}
				}
...
///////////////////////    			
				...
				try {
					...
					Object filename_tFileInputDelimited_1 = context.folderName + "/files/TablesToDump/Quarter.csv";
					...
					try {
						fid_tFileInputDelimited_1 = new org.talend.fileprocess.FileInputDelimited(
								context.folderName + "/files/TablesToDump/Quarter.csv", "ISO-8859-15", ";", "\n", true,
					...
					while (fid_tFileInputDelimited_1 != null && fid_tFileInputDelimited_1.nextRecord()) {
						...
						try {
							...
							row1.quarterName = fid_tFileInputDelimited_1.get(columnIndexWithD_tFileInputDelimited_1);
							...
							if (temp.length() > 0) {
								...
								try {
									...
									row1.year = ParserUtils.parseTo_int(temp);
								...
								rowstate_tFileInputDelimited_1.setException(new RuntimeException(
										"Value is empty for column : 'year' in 'row1' connection, value is invalid or this column should be nullable or have a default value."));
...
///////////////////////		
							...
							if (row1.quarterName != null) { //

								strBuffer_tLogRow_1.append(String.valueOf(row1.quarterName));

							} //

							strBuffer_tLogRow_1.append("|");

							strBuffer_tLogRow_1.append(String.valueOf(row1.year));
...
///////////////////////    			
							...
							if (checkCount_tDBOutput_1 > 0) {
								if (row2.quarterName == null) {
									pstmtUpdate_tDBOutput_1.setNull(1, java.sql.Types.VARCHAR);
								} else {
									pstmtUpdate_tDBOutput_1.setString(1, row2.quarterName);
								}

								pstmtUpdate_tDBOutput_1.setInt(2, row2.year);

								if (row2.id == null) {
									pstmtUpdate_tDBOutput_1.setNull(3 + count_tDBOutput_1, java.sql.Types.VARCHAR);
								} else {
									pstmtUpdate_tDBOutput_1.setString(3 + count_tDBOutput_1, row2.id);
								}
								...
								if (row2.quarterName == null) {
									...
									pstmtInsert_tDBOutput_1.setString(2, row2.quarterName);
								}

								pstmtInsert_tDBOutput_1.setInt(3, row2.year);
					...
					if (!((Object) (context.folderName
							+ "/files/TablesToDump/Quarter.csv") instanceof java.io.InputStream)) {
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final dumpQuarter dumpQuarterClass = new dumpQuarter();

		int exitCode = dumpQuarterClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = dumpQuarter.class.getClassLoader()
					.getResourceAsStream("projetsid/dumpquarter_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = dumpQuarter.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : dumpQuarter");
		}
...
/************************************************************************************************
 * 75980 characters generated by Talend Open Studio for Data Integration on the
 * 8 mai 2024 à 18:45:12 CEST
 ************************************************************************************************/