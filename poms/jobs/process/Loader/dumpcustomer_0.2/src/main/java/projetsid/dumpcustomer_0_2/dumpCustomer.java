...
package projetsid.dumpcustomer_0_2;
...
/**
 * Job: dumpCustomer Purpose: load le product<br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class dumpCustomer implements TalendJob {
	...
	private final String jobName = "dumpCustomer";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(dumpCustomer.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:4smxG3UUYTIbNGaSHOfYjjK4J/CxUd7Ueary4OtvUUJerJsMDYSXTtTUahBzovBl600DKSEjv5DQNPYB");
	...
	public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
		final static byte[] commonByteArrayLock_PROJETSID_dumpCustomer = new byte[0];
		static byte[] commonByteArray_PROJETSID_dumpCustomer = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PROJETSID_dumpCustomer.length) {
					if (length < 1024 && commonByteArray_PROJETSID_dumpCustomer.length == 0) {
						commonByteArray_PROJETSID_dumpCustomer = new byte[1024];
					} else {
						commonByteArray_PROJETSID_dumpCustomer = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PROJETSID_dumpCustomer, 0, length);
				strReturn = new String(commonByteArray_PROJETSID_dumpCustomer, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PROJETSID_dumpCustomer, 0, length);
				strReturn = new String(commonByteArray_PROJETSID_dumpCustomer, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PROJETSID_dumpCustomer) {
...
///////////////////////    			
				...
				try {
					...
					Object filename_tFileInputDelimited_1 = context.folderName + "/files/TablesToDump/Customer.csv";
					...
					try {
						fid_tFileInputDelimited_1 = new org.talend.fileprocess.FileInputDelimited(
								context.folderName + "/files/TablesToDump/Customer.csv", "UTF-8", ";", "\n", true, 1, 0,
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									row1.Birth_Years = ParserUtils.parseTo_Integer(temp);
									...
									rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"Birth_Years", "row1", temp, ex_tFileInputDelimited_1),
											ex_tFileInputDelimited_1));
								}

							} else {

								row1.Birth_Years = null;
...
///////////////////////		
							...
							if (row1.Gender != null) { //
								...
								strBuffer_tLogRow_1.append(String.valueOf(row1.Gender));

							} //

							strBuffer_tLogRow_1.append("|");

							if (row1.Birth_Years != null) { //

								strBuffer_tLogRow_1.append(String.valueOf(row1.Birth_Years));
...
///////////////////////    			
							...
							if (row2.Customer_ID == null) {
								...
								pstmt_tDBOutput_1.setString(1, row2.Customer_ID);
							}
							...
							if (checkCount_tDBOutput_1 > 0) {
								if (row2.Gender == null) {
									...
									if (row2.Gender == null) {
										...
										pstmtUpdate_tDBOutput_1.setString(1, String.valueOf(row2.Gender));
									}
								}

								if (row2.Birth_Years == null) {
									pstmtUpdate_tDBOutput_1.setNull(2, java.sql.Types.INTEGER);
								} else {
									pstmtUpdate_tDBOutput_1.setInt(2, row2.Birth_Years);
								}
								...
								if (row2.Customer_ID == null) {
									...
									pstmtInsert_tDBOutput_1.setString(1, row2.Customer_ID);
								}
								...
								if (row2.Gender == null) {
									...
									if (row2.Gender == null) {
										...
										pstmtInsert_tDBOutput_1.setString(2, String.valueOf(row2.Gender));
									}
								}
								...
								if (row2.Birth_Years == null) {
									...
									pstmtInsert_tDBOutput_1.setInt(3, row2.Birth_Years);
								}
					...
					if (!((Object) (context.folderName
							+ "/files/TablesToDump/Customer.csv") instanceof java.io.InputStream)) {
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final dumpCustomer dumpCustomerClass = new dumpCustomer();

		int exitCode = dumpCustomerClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = dumpCustomer.class.getClassLoader()
					.getResourceAsStream("projetsid/dumpcustomer_0_2/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = dumpCustomer.class.getClassLoader()
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : dumpCustomer");
		}
...
/************************************************************************************************
 * 82925 characters generated by Talend Open Studio for Data Integration on the
 * 9 mai 2024 à 18:27:43 CEST
 ************************************************************************************************/