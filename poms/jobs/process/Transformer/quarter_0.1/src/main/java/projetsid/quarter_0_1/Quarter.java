...
/**
 * Job: Quarter Purpose: extraire la table pays du csv<br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class Quarter implements TalendJob {
	...
	private final String jobName = "Quarter";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Quarter.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row11Struct implements routines.system.IPersistableRow<row11Struct> {
		final static byte[] commonByteArrayLock_PROJETSID_Quarter = new byte[0];
		static byte[] commonByteArray_PROJETSID_Quarter = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PROJETSID_Quarter.length) {
					if (length < 1024 && commonByteArray_PROJETSID_Quarter.length == 0) {
						commonByteArray_PROJETSID_Quarter = new byte[1024];
					} else {
						commonByteArray_PROJETSID_Quarter = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PROJETSID_Quarter, 0, length);
				strReturn = new String(commonByteArray_PROJETSID_Quarter, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PROJETSID_Quarter, 0, length);
				strReturn = new String(commonByteArray_PROJETSID_Quarter, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PROJETSID_Quarter) {
...
// ###############################      
	...
	public static void main(String[] args) {
		final Quarter QuarterClass = new Quarter();

		int exitCode = QuarterClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Quarter.class.getClassLoader()
					.getResourceAsStream("projetsid/quarter_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Quarter.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Quarter");
		}
